...
class Remark{
	...
	public function imageWaterMark($groundImage, $waterPos = 0, $waterImage = "", $waterText = "", $textFont = 5, $textColor = "#FF0000") {
	    ...
	    if ($isWaterImage) {//图片水印
	        ...
	        $temp = imagettfbbox( ceil($textFont * 5), 0, str_replace("\\",'/', $this->options['fonttype']), $waterText); //取得使用 TrueType 字体的文本的范围

	        $w = $temp[2] - $temp[6];
	        $h = abs($temp[7]) - ($temp[3] > 0 ? $temp[3] : 0);
	        $w += 2;
	        $h += 2;
	    ...
	    switch ($waterPos) {
	        ...
	        case 9://9为底端居右
	            ...
	            $posX = $ground_w - $w + 100;   // -10 是距离右侧10px 可以自己调节
